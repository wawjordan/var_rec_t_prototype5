...
contains
  ...
  function constructor( grid, block_num, n_dim, degree, n_var ) result(this)
    ...
    real(dp), dimension(n_dim) :: h_ref
    integer,  dimension(2*n_dim) :: nbor_block, nbor_cell_idx, nbor_face_id
    integer :: n, n_interior, n_cell_nodes
    call this%destroy()
    allocate( this%n_cells( n_dim ) )
    this%n_cells        = grid%gblock(block_num)%n_cells(1:n_dim)
    this%n_cells_total  = product(this%n_cells)
    ...
    do n = 1,this%n_cells_total
      idx_tmp(1:n_dim) = global2local_bnd( n, lo(1:n_dim), hi1(1:n_dim) )
      call cell_face_nbors( n_dim, n, lo(1:n_dim), hi1(1:n_dim),               &
                            nbor_cell_idx, nbor_face_id, n_interior )
      nbor_block = block_num
      nodes = pack_cell_node_coords( idx_tmp, lo, hi2, n_skip,                 &
                                     grid%gblock(block_num)%node_coords )
      h_ref = maximal_extents( n_dim, n_cell_nodes, nodes(1:n_dim,:) )
      associate( quad => grid%gblock(block_num)%grid_vars%quad( idx_tmp(1),    &
                                                                idx_tmp(2),    &
                                                                idx_tmp(3) ) )
        this%cells(n) = var_rec_cell_t( this%p,                                &
                                        block_num,                             &
                                        n,                                     &
                                        nbor_block,                            &
                                        nbor_cell_idx,                         &
  ...
  pure function get_cell_RHS( this, lin_idx, term_start, term_end,             &
                              n_var, var_idx ) result(b)
    use set_constants,           only : zero
    class(var_rec_block_t),     intent(in) :: this
    integer,                    intent(in) :: lin_idx, term_start, term_end
  ...
  pure function get_cell_residual( this, lin_idx, term_start, term_end,        &
    ...
    associate( A  => this%cells(i)%A,  &
      ...
      residual = residual - this%get_cell_RHS( i, term_start, term_end,        &
                                               n_var, var_idx )
      do jj = 1,n_interior
        j = this%cells(i)%nbor_idx(jj)
        do v = 1,n_var
  ...
  pure function get_error_norm(this,gblock,var_idx,n_terms,norms,eval_fun)     &
                                                              result(err_norms)
    use set_constants,           only : zero
    use index_conversion,        only : global2local
    use grid_derived_type,       only : grid_block
    ...
    do i = degree_start, degree_end
      ...
      do blk = 1, this%n_blocks
        call this%b(blk)%init_cells( grid, term_start, term_end,               &
                                     n_vars, [(n,n=1,n_vars)] )
        call this%b(blk)%solve( term_start, term_end, n_vars, [(n,n=1,n_vars)],&
...
contains
  ...
  !   use message,              only : error_message
  !   type(grid_type),           intent(in) :: grid
  !   integer,                   intent(in)  :: n_dim, n_levels, coarsen_factor, n_vars, degree
  !   type(var_rec_t),           intent(out) :: rec
  !   class(func_h_t), optional, intent(in)  :: eval_fun
  !   character(*),    optional, intent(in) :: result_file
  !   character(*), parameter :: routine_name = 'run_grid_convergence'
  ...
  ! end subroutine run_grid_convergence
